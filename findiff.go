...
func Make2D(i, j int) []ProtoCalc {
	scale := angbohr * angbohr / (4 * deltas[i-1] * deltas[j-1])
	switch {
	case i == j:
...
func Make3D(i, j, k int) []ProtoCalc {
	scale := angbohr * angbohr * angbohr / (8 * deltas[i-1] * deltas[j-1] * deltas[k-1])
	switch {
	case i == j && i == k:
...
func Make4D(i, j, k, l int) []ProtoCalc {
	scale := angbohr * angbohr * angbohr * angbohr / (16 * deltas[i-1] * deltas[j-1] * deltas[k-1] * deltas[l-1])
	switch {
	// all the same
	case i == j && i == k && i == l:
		return []ProtoCalc{
